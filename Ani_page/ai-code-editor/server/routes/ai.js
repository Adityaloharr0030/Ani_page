...
router.post("/auth/validate-keys", auth, async (req, res) => {
  ...
  try {
    ...
    // Disable caching for now to ensure fresh responses
...
router.post("/generate-code", auth, validatePrompt, async (req, res) => {
  try {
    ...
    const useGroq = hasGroq && (
      ...
      preferredProvider === "auto" ||
      (allowFallback && !hasOpenAI && !hasPerplexity)
    );

    // If Groq is available (preferred for speed and free tier), use it!
    if (useGroq) {
      try {
...
1. **Chain of Thought**: Before writing code, briefly plan your approach in a comment block.
   - Use correct comment syntax for ${language} (e.g., <!-- --> for HTML, // for JS, # for Python).
...
Generate production-ready code that developers would be proud to use.`;
        ...
        const resp = await callAI('code-generation', messages, {
          max_tokens: 2048,
          temperature: 0.3
        });

        return res.json({ ok: true, resp, provider: "Groq (Fast & Free)" });
      } catch (groqError) {
        console.log("Groq failed:", groqError.message);
        try {
          const fs = require('fs');
          fs.appendFileSync('debug-error.log', `Groq Error: ${groqError.message}\nStack: ${groqError.stack}\n`);
        } catch (e) { }
        // Continue to fallback if Groq fails
      }
    }

    if (!hasOpenAI && !hasPerplexity && !hasGroq) {
    ...
    const preferPerplexity = preferredProvider === "perplexity";
    const preferOpenAI = preferredProvider === "openai";

    if (preferPerplexity && hasPerplexity) {
      try {
        ...
        res.json({
          ...
          resp,
          provider: "perplexity",
          note: "Generated using Perplexity",
        });
        return;
      } catch (perplexityError) {
        if (!allowFallback && !hasOpenAI) {
          return res.status(500).json({ ok: false, error: perplexityError.message });
        }
      }
    }

    if ((hasOpenAI && !preferPerplexity) || (preferOpenAI && hasOpenAI)) {
      try {
        const systemPrompt = `You are an expert senior programmer with 15+ years of experience.
...
Generate production-ready code that developers would be proud to use.`;
        ...
        if (hasPerplexity && allowFallback && !preferOpenAI) {
          try {
            const resp = await callPerplexityForCode({
              prompt: sanitized,
              language,
    ...
    if (hasPerplexity && (!hasOpenAI || preferPerplexity)) {
      try {
        const resp = await callPerplexityForCode({
          prompt: sanitized,
          language,
...
1. **Purpose**: What does this code do? (EL5: Explain like I'm 5)
2. **Analogy**: Use a simple real-world analogy to explain the concept.
3. **Key Components**: Main functions/classes and their responsibilities
4. **How It Works**: Step-by-step execution flow
5. **Important Details**: Key logic and algorithms used
6. **Best Practices**: Whether code follows best practices
7. **Potential Issues**: Any bugs, security issues, or performance concerns
8. **Improvements**: Suggestions for optimization or better approaches
...
\`\`\``;
      ...
      if (preferredProvider !== "perplexity") {
        ...
        return res.json({ ok: true, resp, provider: "openai" });
      }
    } catch (openaiError) {
      if (preferredProvider !== "openai" && allowFallback) {
        ...
        return res.json({ ok: true, resp, provider: "perplexity" });
      }
      return res.status(500).json({ ok: false, error: openaiError.message });
    }
  } catch (error) {
...
Your Task:
1. **Root Cause Analysis**: Explain WHY the bug is happening.
2. **Identify All Issues**: Logic errors, syntax errors, runtime errors, edge cases
3. **Fix Each Issue**: Provide corrected code with explanations
4. **Preserve Functionality**: Keep the original intent and behavior
5. **Improve Quality**: While fixing, also improve code quality
6. **Document Changes**: Add comments explaining what was fixed
...
2. **Big O Analysis**: Analyze time and space complexity before and after.
3. Apply advanced optimization techniques
4. Maintain or improve functionality
5. Add detailed comments explaining each optimization
6. Show performance improvement metrics if applicable
